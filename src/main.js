...
const CARD = `<article class="card card--black">
...
const LOAD_MORE = `<button class="load-more" type="button">load more</button>`;

const renderComponent = (componentToRender, tagToInsert) => {
  tagToInsert.innerHTML += componentToRender;
};

renderComponent(MENU, mainControlTag);
renderComponent(SEARCH, mainTag);
renderComponent(FILTERS, mainTag);
renderComponent(BOARD, mainTag);

const boardTasksTag = document.querySelector(`.board__tasks`);

for (let i = 0; i < 4; i++) {
  renderComponent(CARD, boardTasksTag);
}

const boardTag = document.querySelector(`.board`);
renderComponent(LOAD_MORE, boardTag);
